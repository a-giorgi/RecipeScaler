...
import com.example.proportion.dialogs.AddElementDialog;
import com.example.proportion.dialogs.TextDialog;
import com.example.proportion.dialogs.TotalDialog;
...
public class MainActivity extends AppCompatActivity implements AddElementDialog.DialogCallback,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listContentAdapter = new RecyclerView.Adapter<ListElementViewHolder>() {
            ...
            @NonNull
            @Override
            public ListElementViewHolder onCreateViewHolder(@NonNull ViewGroup parent,
                                                            int viewType) {
    ...
    public void onTextConfirmed(String text) {
        try {
            ...
            convertStringToItems(text,tags);
    ...
    private void launchImageCropper(Uri uri) {
        ...
        CropImageContractOptions cropImageContractOptions = new CropImageContractOptions(uri,
                cropImageOptions);
    ...
    ActivityResultLauncher<CropImageContractOptions> cropImage =
        ...
        if (result.isSuccessful()) {
            Bitmap cropped = BitmapFactory.decodeFile(result.getUriFilePath(getApplicationContext(),
                    true));
            performOcr(cropped);
        }
    });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_scan) {
            if (!verifyPermissions()) {
                ActivityCompat.requestPermissions(MainActivity.this, PERMISSIONS_REQUIRED,
    ...
    public void convertStringToItems(String string, String[] tags){
        ...
        for(int i = 0; i<words.length; i++){
            if(tags[i].equals("QUANTITY") && lastTag.equals("NONE")){
                lastTag = "QUANTITY";
                lastIdx = i;
            }else if(tags[i].equals("FOOD") && lastTag.equals("NONE")){
                lastTag = "FOOD";
                lastIdx = i;
            }else if(tags[i].equals("QUANTITY") && lastTag.equals("FOOD")){
                Element newElement = new Element(words[lastIdx],Float.parseFloat(words[i]));
                newElement.setInitialValue(Float.parseFloat(words[i])/currentScaleFactor);
                listContentElements.add(newElement);
                listContentAdapter.notifyItemInserted(listContentElements.size() - 1);
                lastTag = "NONE";
                lastIdx = -1;
            }else if(tags[i].equals("FOOD") && lastTag.equals("QUANTITY")){
                Element newElement = new Element(words[i],Float.parseFloat(words[lastIdx]));
                newElement.setInitialValue(Float.parseFloat(words[lastIdx])/currentScaleFactor);
                listContentElements.add(newElement);
                listContentAdapter.notifyItemInserted(listContentElements.size() - 1);
                lastTag = "NONE";
                lastIdx = -1;
